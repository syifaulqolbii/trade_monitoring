...
export interface Metrics {
  balance: number;
  equity: number;
  growthPct: number; // equity vs awal, exclude depo/WD
  ...
  startBalance: number;
  startEquity: number;
  netCashFlow: number; // Σ deal balance/credit (depo positif, WD negatif)
}
...
/** Konversi lot akun cent ke lot standar (÷¹¹¹).
...
/** Deal setoran / penarikan / bonus / credit — bukan trading.
 *  MT5: type 2 = balance, type 3 = credit. */
export function isCashFlowDeal(d: DealInput): boolean {
  return d.type === 2 || d.type === 3;
}

/** Cumulative cash flow (profit deal balance/credit) sampai waktu t (inklusif). */
export function cashFlowUpTo(deals: DealInput[], t: number): number {
  let sum = 0;
  for (const d of deals) {
    if (!isCashFlowDeal(d)) continue;
    if (asDate(d.time).getTime() <= t) sum += d.profit;
  }
  return sum;
}
...
export function computeMetrics(
  ...
  const lastT = lastSnap?.t ?? 0;
  const firstT = snaps.length > 0 ? snaps[0].t : 0;

  const cfBefore = snaps.length > 0 ? cashFlowUpTo(deals, firstT) : 0;
  const cfAll = cashFlowUpTo(deals, lastT || Number.POSITIVE_INFINITY);
  const adjStart = startEquity - cfBefore;
  const adjEnd = equity - cfAll;

  // ---- drawdown dari equity yang sudah dikurangi cash flow ----
  ...
  const currentDD =
    runningMax > 0 ? ((runningMax - adjEnd) / runningMax) * 100 : 0;

  // ---- growth: perubahan equity SETELAH depo/WD dikeluarkan ----
  const growthPct =
  ...
  for (const c of closed) {
    ...
    if (c.netProfit > 0) m.winRate = (m.winRate ?? 0) + 1;
  }
  for (const d of deals) {
...
export interface GrowthDDPoint {
  t: number; // epoch ms
  growthPct: number; // relatif thd equity pertama (sudah exclude depo/WD)
  ddPct: number; // underwater drawdown NEGATIF
}

/** Kurva pertumbuhan relatif + underwater drawdown per snapshot.
 *  Equity tiap titik dikurangi cumulative cash flow sampai waktu itu.
 *  `allCashFlow` harus berisi SEMUA deal balance/credit akun (tanpa filter
 *  waktu) agar deposit sebelum awal rentang ikut dikoreksi. */
export function growthDrawdownSeries(
  snapshots: SnapshotInput[],
  allCashFlow: DealInput[] = []
): GrowthDDPoint[] {
  const snaps = [...snapshots]
    .map((s) => ({ equity: s.equity, t: asDate(s.createdAt).getTime() }))
    .sort((a, b) => a.t - b.t);
  if (snaps.length === 0) return [];
  const firstAdj = snaps[0].equity - cashFlowUpTo(allCashFlow, snaps[0].t);
  let runningMax = -Infinity;
  const out: GrowthDDPoint[] = [];
  for (const s of snaps) {
    const adj = s.equity - cashFlowUpTo(allCashFlow, s.t);
    if (adj > runningMax) runningMax = adj;
    const growthPct =
      firstAdj !== 0 ? ((adj - firstAdj) / Math.abs(firstAdj)) * 100 : 0;
    const ddPct =
