...
export async function loadAccountMetrics(
  ...
  const [deals, positions, snapshots, allCashFlow] = await Promise.all([
    ...
    // SEMUA deal balance/credit akun (tanpa filter waktu) untuk koreksi
    // growth & drawdown — deposit sebelum awal rentang harus ikut dihitung.
    prisma.deal.findMany({
      where: {
        accountId,
        OR: [{ type: 2 }, { type: 3 }],
  ...
  return {
    ...
    metrics,
    openSummary,
    growthDD: growthDrawdownSeries(snapshots, allCashFlow),
